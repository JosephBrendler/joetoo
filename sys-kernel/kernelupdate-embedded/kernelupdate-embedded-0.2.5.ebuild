# Copyright 2022-2052 Joe Brendler
# Distributed under the terms of the GNU General Public License v2
# kernel builder program for embedded systems (tinkerboard s)

EAPI=8

DESCRIPTION="kernel build program for embedded systems"
HOMEPAGE="https://github.com/joetoo"
SRC_URI=""

LICENSE="MIT"
SLOT="0"
#KEYWORDS="~arm ~amd64 ~arm64"

IUSE="+rk3288-tinker-s rk3399-tinker2-s rk3399-rock-4c-plus bcm2709-rpi-2-b bcm2710-rpi-3-b-plus bcm2711-rpi-4-b bcm2712-rpi-5-b"
BOARDLIST="rk3288-tinker-s rk3399-tinker2-s rk3399-rock-4c-plus bcm2709-rpi-2-b bcm2710-rpi-3-b-plus bcm2711-rpi-4-b bcm2712-rpi-5-b"

# tinker-s is default above, but "at least one of" the options
REQUIRED_USE="
	|| ( ${BOARDLIST} )
	"

# required by Portage, as we have no SRC_URI...
S="${WORKDIR}"

RDEPEND="
	>=sys-devel/crossdev-20230321
	>=dev-util/joetoolkit-0.1.5
	>=joetoo-base/joetoo-meta-0.0.4b
	>=app-admin/eselect-1.4.27-r1
"

DEPEND="${RDEPEND}"

# Install for selected board(s) from above different choices, like joetoo-meta does via pkg_setup(),
# but note that where joetoo-meta is "exactly-one-of" board, this is "at-least-one-of"...
# Therefor, need to do in src_install and use for loop to install selected boards
src_install() {
	einfo "S=${S}"
	einfo "D=${D}"
	einfo "P=${P}"
	einfo "PN=${PN}"
	einfo "PV=${PV}"
	einfo "PVR=${PVR}"
	einfo "FILESDIR=${FILESDIR}"
	einfo "RDEPEND=${RDEPEND}"
	einfo "DEPEND=${DEPEND}"
	einfo "BOARDLIST: [ ${BOARDLIST} ]"

	# install the kernelupdate-embedded script
	elog "Installing script ${PN}..."
	dodir "/usr/sbin/"
	z="${PN}"
	einfo "About to execute command cp -v ${FILESDIR}/${z} ${D}/usr/sbin/${z};"
	cp -v "${FILESDIR}/${z}" "${D}/usr/sbin/${z}";
	# install the README-instructions file for getting sources
	elog "Installing README-instructions file..."
	dodir "/etc/${PN}/"
	z="README-instructions"
	einfo "About to execute command cp -v ${FILESDIR}/${z} ${D}/etc/${PN}/${z};"
	cp -v "${FILESDIR}/${z}" "${D}/etc/${PN}/${z}";
	elog "Done installing script and README"

	elog "Installing configuration files for selected boards..."
	for board in ${BOARDLIST};
	do
		if use ${board}; then
			elog "USE flag \"${board}\" selected ..."
			dodir "/etc/${PN}/${board}/"
			for x in $(find ${FILESDIR}/${board}/ -maxdepth 1 -type f);
			do
				z=$(echo ${x} | sed "s|${FILESDIR}/${board}/||");
				einfo "About to execute command cp -v "${x}" "${D}"/etc/${PN}/"${z}";"
				cp -v "${x}" "${D}/etc/${PN}/${z}";
			done
			elog "done"
		else
			elog "USE flag \"${board}\" not selected; ${board} configs not copied"
		fi
	done
	# install the joetoo kernelupdate-embedded.conf eselect module
	elog "Installing the joetoo kernelupdate-embedded.conf eselect module..."
	dodir "/usr/share/eselect/modules/"
	z="kernelupdate.eselect"
	einfo "About to execute command cp -v ${FILESDIR}/${z} ${D}/usr/share/eselect/modules/${z};"
	cp -v "${FILESDIR}/${z}" "${D}/usr/share/eselect/modules/${z}";
	elog "Done installing the joetoo kernelupdate-embedded.conf eselect module."

}

pkg_postinst() {
	elog "S=${S}"
	elog "D=${D}"
	elog "P=${P}"
	elog "PN=${PN}"
	elog "PV=${PV}"
	elog "PVR=${PVR}"
	elog "FILESDIR=${FILESDIR}"
	elog "RDEPEND=${RDEPEND}"
	elog "DEPEND=${DEPEND}"
	elog "BOARDLIST=${BOARDLIST}"
	elog ""
	elog "${PN} installed"
	elog ""
	elog "Version 0.0.5 incorporates ${PN}.conf file(s) and generalizes for multiple models"
	elog "Version 0.0.6 enables use of multiple .conf files and provides examples for models"
	elog "  tinkerboard-s and Pi4b.  You can use/modify, and symlink the one you want"
	elog "  to ${PN}.conf in /etc/${PN}/"
	elog "Version 0.1.0 adds Rock4cplus, Pi 3b+, Pi 5b, and consolidates src_install() "
	elog "Version 0.1.1-4 provide bugfixes"
	elog "Version 0.1.5 installs README-instructions in /etc/${PN}/ vs /usr/sbin/"
	elog "Version 0.2.0 splits publish tarball and ebuild functions, either can be skipped"
	elog "  makeopts and features are now .conf variables"
	elog "  command sequence is now resumable at arbitrary step number"
	elog "Version 0.2.1 adds support for bcm2709-rpi-2-b (Raspberry Pi 2 B)"
	elog "Version 0.2.2 provides bugfixes"
	elog "Version 0.2.3 adds a joetoo kernelupdate-embedded.conf eselect module"
	elog "Version 0.2.4 drops pre-pub steps (skip publish_tarball if user uploads w git-desktop)"
	elog "Version 0.2.5 fine-tunes dtb file and overlay installation"
	elog ""
	elog "This software is still preliminary.  Please report bugs to the maintainer."
	elog ""
	elog "Thank you for using ${PN}"
}
