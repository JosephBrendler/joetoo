...
### going back to tbz2 iaw these update instructions - can't remember where the tar.gz comes from
# (if it is kernel.org's tar.xz then we don't even need to host it on raspi56406, and can just
#  get it from e.g. https://cdn.kernel.org/pub/linux/kernel/v6.x/linux-6.12.62.tar.xz)
# (if it is coming from raspbian build, I've lost track of how... )
...
#SRC_URI="https://raspi56406.brendler/amlogic-sources/linux-${PV}-amlogic.tar.gz"
...
src_install() {
	...
	target="/usr/src/"
	einfo "Installing (ins) sources [ ${My_P} ] into ${target} ..."
	insinto "${target}"
	# copy sources to temp directory T under a new name
	einfo "Copying sources ${S}/${My_P} to ${T}/"
	cp -r "${S}/${My_P}" "${T}/${My_P}-amlogic"
	# now install the renamed directory in ${target}
	doins -r "${T}/${My_P}-amlogic"
	elog "Installed sources [ ${My_P} ] into ${target}"
	if use symlink ; then
		einfo "symlink USE flag is set, installing symlink ..."
		dosym ${target%/}/${My_P} ${target%/}/linux
		elog "Installed symlink in ${target}  ${My_P} <-- linux"
	fi
	einfo "sys-apps/portage and some other packages want to inspect kernel"
	einfo "configuration by reading .config in ${target%/}/linux/ ..."
	if use config; then
		# put config in sources
		einfo "Installing (ins) .config into ${target%/}/${My_P}/"
		...
		elog "Installed .config in ${target%/}/${My_P}"
	else
