...
SRC_URI="https://raw.githubusercontent.com/JosephBrendler/myUtilities/master/mkinitramfs-6.5.9999.tbz2"
...
src_install() {
	...
	elog "version 6.1 corrects grub-related bugs"
	elog "version 6.2 patches for a rare issuxe with cryptsetup failure for missing file"
	elog "  (libgcc_s.so.1) needed for cancel_pthreads"
	elog "version 6.3 preserves init.conf by moving both config files to /etc/mkinitramfs/"
	elog "version 6.4 puts ckinitramfs in /usr/bin instead of /usr/local/bin"
	elog" version 6.5 drops lvmconf from dynexecutables and adds some lvm linked files"
	elog ""
