...
pkg_postinst() {
	...
	elog "Version 0.0.5 adds/tweaks a nextcloud_upgrade utility"
	elog "Version 0.1.8 moved kernelupdate-tinker utility to a separate eponymous package"
	elog "Version 0.1.9 adds tool to remove bogus binary packages on \"Destination exists\""
	elog "Versions up to 0.1.14 tweak tools and move some tools to/from other packages"
	elog "Version 0.2.0 improves the nextcloud_upgrade utility"
	elog "Version 0.3.0 introduces nuoromis infrastructure related code"
	elog "Version 0.3.1 adds server cert for tinker05"
	elog "Version 0.3.2 add to x_arg_nobinmergelist list of pkgs to add to package.env"
	elog "Version 0.3.3 updates geolocate utility (ipapi.co vs tools.keycdn.com)"
	elog "Version 0.3.4 updates the loggit utility"
	elog "Version 0.3.5 updates the ipset_countrycode_load, update_ipdeny tools and cronjobs"
	elog "Version 0.3.6 removes vpn/svc/temp tools now provided by headless-mata packages"
	elog "Version 0.3.7 adds alias mb='mount /boot' to bashrc_aliases_include_joe_brendler"
	elog "Version 0.3.8/9 add/update utilities"
	elog "Version 0.3.10 removed kernelupdate (temp kernelupdate-old)"
	elog "Ver 0.4.0 adds git, nextcloud tools, archives kernel tools, aligns # w joetoo packages"
	elog ""
	elog "To Do:"
