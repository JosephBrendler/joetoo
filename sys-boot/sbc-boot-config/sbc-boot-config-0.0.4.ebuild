...
#			newins "${FILESDIR}/${board}/config.txt" "config.txt" ;
			# install cmdline.txt, config.txy and any other (e.g. README.txt) .txt files
			for x in $(find ${FILESDIR}/${board}/ -iname '*.txt'); do
				newins "${x}" "$(basename ${x})" ;
				elog "Installed (ins) ${x} into /boot/"
			done ;
...
#			newins "${FILESDIR}/${board}/boot.scr" "boot.scr" ;
			...
			for x in $(find ${FILESDIR}/${board}/ -iname 'boot.*'); do
				newins "${x}" "$(basename ${x})"
				elog "Installed (ins) ${x} into /boot/"
			done ;
