...
DESCRIPTION="OrangePi 5/5b u-boot files for 64-bit mode"
...
src_install() {
	...
	dodir /boot
		for x in $(find ${FILESDIR}/ -maxdepth 1 -type f | grep -v config_protect);
                do
			z=$(echo $(basename ${x}) | sed "s|rk3588s-orangepi-5_||");
			cp -v "${x}" "${D}/boot/${z}"
		done
	dodir /boot/u-boot_reflash
		for x in $(find ${FILESDIR}/rk3588s-orangepi-5_u-boot_reflash/ -maxdepth 1 -type f);
		do
