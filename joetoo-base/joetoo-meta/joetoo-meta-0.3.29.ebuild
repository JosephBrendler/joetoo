...
IUSE="
	...
	-samba
	-sbc -bcm2712-rpi-5-b -bcm2711-rpi-4-b -bcm2710-rpi-3-b-plus -bcm2709-rpi-2-b -rk3288-tinker-s -rk3399-rock-pi-4c-plus -rk3399-tinker-2-s
	"
...
src_install() {
	...
	insinto "/etc/openvpn/openvpnkeys_2024/"
	...
	insinto "/etc/portage/package.accept_keywords/"
		if use sbc ; then
			case $board in
				"bcm2712-rpi-5-b"|"bcm2711-rpi-4-b"|"bcm2710-rpi-3-b-plus"|"rk3399-rock-pi-4c-plus"|"rk3399-tinker-2-s")
					# arch=arm64
	...
	insinto "/etc/portage/binrepos.conf/"
		if use sbc; then
			case $board in
				...
				"rk3399-rock-pi-4c-plus"|"rk3288-tinker-s"|"rk3399-tinker-2-s")
					# nothing, yet
					;;
			esac
		else
...
pkg_postinst() {
	...
	elog "version 0.3.5 updates package.env (for pkgs that have failed to compile with distcc)"
	elog "version 0.3.6 provides per-package /etc/portage/env/tmpdir_on_rootfs.conf "
	...
	elog "version 0.3.13-14 update package.env (for additional pkgs that have failed to compile with distcc)"
	elog "version 0.3.15 provides a joetoo-default /etc/ssl/openssl.conf"
	elog "version 0.3.16-17 update package.env (for additional pkgs that have failed to compile with distcc)"
	elog "version 0.3.18 changes default time-keeper from ntp-client to chrony and adds configs for both"
	elog "version 0.3.19 updates package.env (for additional pkgs that have failed to compile with distcc)"
	...
	elog "version 0.3.22 updates package.env (for additional pkgs that have failed to compile with distcc)"
	elog "version 0.3.23 updates package.env, openvpn parts, and config_protect"
	elog "version 0.3.24-27 update package.env (for additional pkgs that have failed to compile with distcc)"
	elog "version 0.3.28/29 introduces support for rk3399-tinker-2-s"
	elog ""
