...
SLOT="0"
IUSE="
	+innercore +gpio +joetoo -boot-fw -upstream
...
# disable upstream firmware choice for rockchip boards
RESTRICT="
	rk3288-tinker-s? ( upstream )
	rk3399-rock-pi-4c-plus? ( upstream )
	rk3399-tinker-2-s? ( upstream )
	rk3588s-orangepi-5? ( upstream )
"
...
RDEPEND="
	...
	boot-fw? (
		bcm2712-rpi-5-b? (
			upstream? ( >=sys-boot/rpi5-64bit-firmware-1.20230405 )
			!upstream? ( sys-kernel/linux-bcm2712-rpi-5-b_kernel_image )
		)
		bcm2711-rpi-4-b? (
			upstream? ( >=sys-boot/rpi3-64bit-firmware-1.20220331 )
			!upstream? ( sys-kernel/linux-bcm2711-rpi-4-b_kernel_image )
		)
		bcm2710-rpi-3-b-plus? (
			upstream? ( >=sys-boot/rpi3-64bit-firmware-1.20220331 )
			!upstream? ( sys-kernel/linux-bcm2710-rpi-3-b-plus_kernel_image )
		)
		bcm2709-rpi-2-b? (
