...
# version 0.2.0 swithced to board model spec USE selection, if embedded
IUSE="
	+innercore
	...
	+jus +cloudsync +script_header_brendlefly
	+compareConfigs +Terminal
	-gentoo_pv_kernel_image
	-samba
	-embedded -bcm2711-rpi-4-b -bcm2710-rpi-3-b-plus -rk3288-tinker-s -rk3399-rock-pi-4c-plus
	"
...
REQUIRED_USE="
	...
	"
# disable mkinitramfs if embedded (raspberry or tinker board) is selected
RESTRICT="
	embedded? ( mkinitramfs )
	"
...
RDEPEND="
	...
	innercore? (
		...
		embedded? ( >=dev-lang/rust-bin-1.66.1-r1 )
		!embedded? (
...
pkg_setup() {
	# for embedded systems we need to know which board we are using
